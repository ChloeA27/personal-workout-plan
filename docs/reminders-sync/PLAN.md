...
- [ ] **（未来想做）自建待办 App**：不满足于 Google Tasks 的简陋 UI 的话，可以参照 HealthExporter 的模式自己写一个 SwiftUI App + WidgetKit 小组件，数据直接从这个仓库拉取，用本地通知做提醒，完全不受第三方平台限制。工程量比复用 Google Tasks 大不少（要自己做列表 UI、widget、通知调度、数据同步），且同样要面对免费签名 7 天过期的问题。等 Google Tasks 这条线跑稳、训练计划内容定下来之后再评估要不要做
